...
(function () {
  ...
  Object.assign(canvas.style, {
    ...
    height:        "100%",
    pointerEvents: "none",
    zIndex:        "1",            // above body background, below hand-pip canvas
  });
  // Insert before all other body children so it sits behind the pip
  ...
  function glsl(id) {
    ...
    if (!el) { console.error("[webgl-particles] Shader not found in DOM:", id); return ""; }
  ...
  function compileShader(type, src) {
    ...
    if (!gl.getShaderParameter(sh, gl.COMPILE_STATUS)) {
      console.error("[webgl-particles] Shader compile error:\n", gl.getShaderInfoLog(sh));
      gl.deleteShader(sh);
      return null;
    }
    return sh;
  ...
  const updU    = locs(updateProg, [
    "uTime","uResolution","uNumTips","uTips","uFlows",
    "uFoxGesture","uHandMoving","uVolumeSpike",
  ]);
